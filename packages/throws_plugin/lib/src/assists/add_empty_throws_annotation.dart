...
class AddEmptyThrowsAnnotationAssist extends ResolvedCorrectionProducer {
  static const AssistKind _assistKind = AssistKind(
    'throws.assist.addEmptyThrowsAnnotation',
    DartFixKindPriority.standard,
    'Add @${ThrowsAnnotation.name}',
  );

  AddEmptyThrowsAnnotationAssist({required super.context});
  ...
  Future<void> compute(ChangeBuilder builder) async {
    ...
    await builder.addDartFileEdit(file, (builder) {
      final throwsRef = builder.importThrows(isCapitalized: false);
      final indent = utils.getLinePrefix(offset);
      final insertOffset = offset - indent.length;
      builder.addInsertion(insertOffset, (builder) {
        builder.writeln('$indent@$throwsRef');
      });
    });
  }
}
